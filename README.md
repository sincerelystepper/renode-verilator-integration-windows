...
- [The Seven Barriers We Broke](#the-seven-barriers-we-broke)
- [Architecture Overview](#architecture-overview)
- [Quick Start](#quick-start)
- [Manual Build Walkthrough](#manual-build-walkthrough)
- [Repository Structure](#repository-structure)
- [Integrating Your Own HDL Design](#integrating-your-own-hdl-design)
...
The enumerator file itself contains action type constants that must match Renode's C# `ActionType` enum exactly — values such as `tickClock == 1`, `writeRequest == 2`, `readRequest == 3`, `interrupt == 6`, and `ok == 9`. Without this file, the `Action` enum is empty, and every reference to `Action::tickClock`, `Action::writeRequest`, and similar identifiers throughout the codebase becomes an undefined symbol.
...
**Fix applied in `BUILD.sh`:** Bypass both `make` and `ar` entirely. The build script enumerates every Verilator-generated `.cpp` file explicitly and compiles each one with a direct `g++ -c` invocation. The resulting `.o` files are then passed individually to the final link step. The file list is:
